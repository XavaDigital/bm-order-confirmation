...
describe('RosterLinkPanel', () => {
  ...
  it('generating a link POSTs to the roster token endpoint and displays the returned url', async () => {
    ...
    const { fetchMock } = installMockFetch([
      {
        match: '/api/admin/orders/order-1/roster/token',
        method: 'POST',
        response: { token: 'raw-token', url: 'http://localhost/o/roster/raw-token' },
      },
    ]);
    renderPanel({ hasActiveToken: false });
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/admin/orders/order-1/roster/token',
  ...
  it('revoking a link DELETEs the roster token endpoint after confirming, and hides the Revoke button', async () => {
    const user = userEvent.setup();
    const { fetchMock } = installMockFetch([
      { match: '/api/admin/orders/order-1/roster/token', method: 'DELETE', response: {} },
    ]);
    renderPanel({ hasActiveToken: true });
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/admin/orders/order-1/roster/token', { method: 'DELETE' });
  ...
  it('locking the roster POSTs to the lock endpoint', async () => {
    const user = userEvent.setup();
    const { fetchMock } = installMockFetch([
      { match: '/api/admin/orders/order-1/roster/lock', method: 'POST', response: { ok: true } },
    ]);
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/admin/orders/order-1/roster/lock',
      expect.objectContaining({ method: 'POST' }),
    );
  ...
  it('unlocking the roster DELETEs the lock endpoint', async () => {
    const user = userEvent.setup();
    const { fetchMock } = installMockFetch([
      { match: '/api/admin/orders/order-1/roster/lock', method: 'DELETE', response: { ok: true } },
    ]);
    renderPanel({ locked: true });

    await user.click(screen.getByRole('switch'));

    expect(fetchMock).toHaveBeenCalledWith('/api/admin/orders/order-1/roster/lock', { method: 'DELETE' });
  ...
  it('emailing the roster link POSTs to the send-link endpoint and shows the returned url', async () => {
    ...
    const { fetchMock } = installMockFetch([
      {
        match: '/api/admin/orders/order-1/roster/send-link',
        method: 'POST',
        response: { url: 'http://localhost/o/roster/raw-token' },
      },
    ]);
    renderPanel({ hasActiveToken: false });
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/admin/orders/order-1/roster/send-link',
      expect.objectContaining({ method: 'POST' }),
    );
