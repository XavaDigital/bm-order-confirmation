...
const { sendMagicLink, isEmailConfigured } = vi.hoisted(() => ({
  sendMagicLink: vi.fn().mockResolvedValue(undefined),
  isEmailConfigured: vi.fn().mockReturnValue(true),
}));

vi.mock('@/lib/email', () => ({ sendMagicLink, isEmailConfigured }));
...
function postRequest() {
  return new NextRequest('http://localhost/api/admin/orders/x/send-link', { method: 'POST' });
}

const UNKNOWN_ID = '00000000-0000-0000-0000-000000000000';

describe('POST /api/admin/orders/[id]/send-link', () => {
  ...
  it('sends a non-revision email for a fresh order and returns the url', async () => {
    ...
    expect(sendMagicLink.mock.calls[0][0]).toMatchObject({
      to: 'jane@example.com',
      isRevision: false,
    });
  });

  it('sends a revision email with the prior comment/count for a changes_requested order', async () => {
    const created = await createOrder(minimalOrderInput());
    await db.update(schema.orders).set({ status: 'changes_requested' }).where(eq(schema.orders.id, created.orderId));
    await db.insert(schema.domainEvents).values({
      aggregateType: 'order',
      aggregateId: created.orderId,
      eventType: 'order.changes_requested',
      payload: { comment: 'Please make it bigger' },
    });
    ...
    expect(sendMagicLink.mock.calls[0][0]).toMatchObject({
      isRevision: true,
      priorComment: 'Please make it bigger',
      revisionNumber: 1,
    });
  });

  it('records a link.emailed audit event', async () => {
    const created = await createOrder(minimalOrderInput());
  ...
  it('returns 500 with the underlying message when sendMagicLink throws', async () => {
    sendMagicLink.mockRejectedValueOnce(new Error('SMTP exploded'));
    const created = await createOrder(minimalOrderInput());
